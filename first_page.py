...
def pg():
    ...
    bg= Image.open('3.jpg')
    k = ImageTk.PhotoImage(bg)
    Label(root, image=k).place(relwidth=1, relheight=1)
...
s.geometry('1200x800')
s.minsize(1000,800)
s.maxsize(1250,20000)
bg=Image.open('4.jpg')
im=ImageTk.PhotoImage(bg)
Label(s,image=im,bg='gray9').place(relwidth=1,relheight=1)
...
Button(s,text='YES',font=("Arial", 16, "bold"),  # Font styling
    ...
    width=4,
    height=1,command=pg).pack(side=LEFT,padx=270,pady=20)

Button(s,text='NO',font=("Arial", 16, "bold"),  # Font styling
    bg="grey9",  # Background color
    fg=rosegold,  # Text color (rosegold)
    activebackground="#1c1c1c",  # Background color when clicked (darker black)
    activeforeground=rosegold,  # Text color when clicked
    ...
    highlightthickness=2,  # Thickness of the border
    width=10,padx=1,
    height=1,command=quit).pack(side='right',padx=270,pady=20)
